...
void Video::open_video(String a_path) {
  ...
  } else if (1 < av_stream_video->duration && av_stream_video->duration < av_format_ctx->duration / AV_TIME_BASE* av_q2d(av_stream_video->r_frame_rate) * 2) {
  ...
  UtilityFunctions::print(av_format_ctx->duration / AV_TIME_BASE* av_q2d(av_stream_video->r_frame_rate));
...
void Video::_get_audio() {
  ...
  double stream_time_base_audio = av_q2d(av_stream_audio->time_base) * 1000.0 *
                                  10000.0; // Converting timebase to ticks
  ...
  response =
      av_seek_frame(av_format_ctx, -1, start_time_audio,
