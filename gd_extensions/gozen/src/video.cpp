...
void Video::open_video(String a_path) {
	...
	const AVCodec *av_codec_video =
		avcodec_find_decoder(av_stream_video->codecpar->codec_id);
	if (!av_codec_video) {
		UtilityFunctions::printerr(
			"Couldn't find any codec decoder for video!");
		close_video();
		return;
	}
	...
	if (av_codec_ctx_video == NULL) {
		UtilityFunctions::printerr(
			"Couldn't allocate codec context for video!");
		close_video();
		return;
	}

	// Copying parameters
	if (avcodec_parameters_to_context(av_codec_ctx_video,
									  av_stream_video->codecpar)) {
	...
	sws_ctx =
		sws_getContext(av_codec_ctx_video->width, av_codec_ctx_video->height,
					   (AVPixelFormat)av_stream_video->codecpar->format,
					   av_codec_ctx_video->width, av_codec_ctx_video->height,
	...
	// Byte_array setup
	byte_array.resize(av_codec_ctx_video->width * av_codec_ctx_video->height *
					  3);
	src_linesize[0] = av_codec_ctx_video->width * 3;

	// Set other variables
	start_time_video =
		av_stream_video->start_time != AV_NOPTS_VALUE
	...
	average_frame_duration =
		...
		av_q2d(
			av_stream_video
				->avg_frame_rate); // eg. 1 sec / 25 fps = 400.000 ticks (40ms)

	// Setting up variables
	UtilityFunctions::print("Checking for variable framerate ...");
	stream_time_base_video = av_q2d(av_stream_video->time_base) * 1000.0 *
							 10000.0; // Converting timebase to ticks

	variable_framerate = av_q2d(av_codec_ctx_video->framerate) !=
						 av_q2d(av_stream_video->avg_frame_rate);
	if (variable_framerate) {
		UtilityFunctions::print(
			"Variable framerate detected, aborting!! (not supported)");
		...
		UtilityFunctions::print("Context framerate:");
		UtilityFunctions::print(av_q2d(av_codec_ctx_video->framerate));
		return;
	}
	...
	if (av_stream_video->nb_frames > 35) {
		UtilityFunctions::print("Total frame based on nb_frames");
		total_frame_number = av_stream_video->nb_frames;
	} else if (1 < av_stream_video->duration &&
			   av_stream_video->duration <
				   av_format_ctx->duration / AV_TIME_BASE *
					   av_q2d(av_stream_video->r_frame_rate) * 2) {
		...
		total_frame_number = av_format_ctx->duration / AV_TIME_BASE;
		total_frame_number *= av_q2d(av_stream_video->r_frame_rate);
	}
	UtilityFunctions::print(total_frame_number);
	UtilityFunctions::print(av_format_ctx->duration / AV_TIME_BASE *
							av_q2d(av_stream_video->r_frame_rate));
...
void Video::print_av_error(const char *a_message) {
	...
	UtilityFunctions::printerr(
		(std::string(a_message) + l_error_buffer).c_str());
}

void Video::_get_audio() {
	// Audio Decoder Setup
	UtilityFunctions::print("Setting up audio decoder ...");

	// Setup Decoder codec context
	const AVCodec *av_codec_audio =
		avcodec_find_decoder(av_stream_audio->codecpar->codec_id);
	if (!av_codec_audio) {
		UtilityFunctions::printerr(
			"Couldn't find any codec decoder for audio!");
		close_video();
		return;
	}

	double stream_time_base_audio = av_q2d(av_stream_audio->time_base) *
									1000.0 *
									10000.0; // Converting timebase to ticks
	int start_time_audio =
		av_stream_audio->start_time != AV_NOPTS_VALUE
			? (long)(av_stream_audio->start_time * stream_time_base_audio)
			: 0;
	...
	if (av_codec_ctx_audio == NULL) {
		UtilityFunctions::printerr(
			"Couldn't allocate codec context for audio!");
		close_video();
		return;
	}

	// Copying parameters
	if (avcodec_parameters_to_context(av_codec_ctx_audio,
									  av_stream_audio->codecpar)) {
	...
	response = av_seek_frame(av_format_ctx, -1, start_time_audio,
							 AVSEEK_FLAG_FRAME | AVSEEK_FLAG_ANY);
	avcodec_flush_buffers(av_codec_ctx_audio);
	if (response < 0) {
		UtilityFunctions::printerr(
			"Can't seek to the beginning of audio stream!");
		return;
	}
	...
	while (av_read_frame(av_format_ctx, av_packet) >= 0) {
		...
		if (av_packet->stream_index == av_stream_audio->index) {
			...
			while (response >= 0) {
				...
				l_av_new_frame->nb_samples =
					swr_get_out_samples(swr_ctx, av_frame->nb_samples);
				...
				size_t l_byte_size = l_av_new_frame->nb_samples *
									 av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
				if (av_codec_ctx_audio->ch_layout.nb_channels >= 2)
					l_byte_size *= 2;

				l_audio_data.resize(l_audio_size + l_byte_size);
				memcpy(&(l_audio_data.ptrw()[l_audio_size]),
					   l_av_new_frame->extended_data[0], l_byte_size);
...
Ref<Image> Video::seek_frame(int a_frame_nr) {
	...
	response = av_seek_frame(av_format_ctx, -1,
							 (start_time_video + frame_timestamp) / 10,
							 AVSEEK_FLAG_FRAME | AVSEEK_FLAG_BACKWARD);
	avcodec_flush_buffers(av_codec_ctx_video);
	if (response < 0) {
	...
	while (true) {
		...
		while (true) {
			...
			sws_scale(sws_ctx, av_frame->data, av_frame->linesize, 0,
					  av_frame->height, l_dest_data, src_linesize);
			l_image->set_data(av_frame->width, av_frame->height, 0,
							  l_image->FORMAT_RGB8, byte_array);
			...
			if (variable_framerate)
				frame_time =
					(float)(av_frame->pts * av_q2d(av_stream_video->time_base));
